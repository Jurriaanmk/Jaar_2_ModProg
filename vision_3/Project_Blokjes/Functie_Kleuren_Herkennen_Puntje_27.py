...
Middenpunt_Print = 0 #1 = aan, 0 = uit. Print in de console de middenpunt waarde, kan gebruikt worden voor het exporteren van de middenpunten van de blokjes
...
def Coordinaten_Doorgeven(InputCoordinaat_X,InputCoordinaat_Y):
    ...
    offset_X_As = 15 #cm
    offset_Y_As = 10 #cm
    #Input / resolutie_foto * Resolutie_DeltaRobot + as_Offset
    ...
    if Middenpunt_Print:
        print('x, y', Output_Coordinaat_X, Output_Coordinaat_Y)
...
def Kleuren_Herkennen(Kleur):
    ...
    if len(Basis_contours) != 0:  # Maak een blauwe contour om alle gevonden contouren
        ...
        Coordinaat_X, Coordinaat_Y = Middenpunt_Bepalen(Basis_contours, Max_Index, angle)
    ...
    return Basis_Mask, Afbeelding, Output_X, Output_Y, angle
...
while Aantal_Cyclussen < 1:
    ...
    for Kleuren_Keys in Kleuren_Bieb.keys():
        ...
        Basis_Masker, Uitslag, Output_X, Output_Y, angle = Kleuren_Herkennen(Kleuren_Keys)

        cv2.imshow(f'Mask_Kleur{Kleuren_Keys}', Basis_Masker)
        cv2.imshow(f'Uitslag_Kleur{Kleuren_Keys}', Uitslag)
        
        print('x ,y, angle:', Output_X, Output_Y, angle)
